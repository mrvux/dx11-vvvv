...
{
    [SceneGraphNode(Name = "KinectDepth", Category = "Devices", SystemName = "flt.sg.kinectdepth", ShowInList = true)]
    public class KinectDepthTexture : ISceneGraphNodeInstance, IResourceObject
    {
        ...
        {
            manager.NewDepthFrame -= manager_NewColorFrame;
            manager = null;
        ...
        public void Update(IRenderObject parent, SceneGraphSettings settings)
        {
            if (this.manager != null)
            {
                if (this.invalidate)
                {
                    ...
                    this.texture.WriteData(settings.RenderContext, this.manager.DepthFrame, 512 * 424 * 2, 2);
