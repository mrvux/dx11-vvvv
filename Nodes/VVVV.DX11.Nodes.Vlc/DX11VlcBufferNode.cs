...
{
    [PluginInfo(Name = "FileStream", Category = "DX11.Buffer", Version = "Vlc", AutoEvaluate = true, Author="vux",Credits="ft")]
    public class VLCNodeBuffer : IPluginEvaluate, IDisposable, IDX11ResourceProvider
    {
        #region Fields
        ...
        [Output("Buffer Out", Order = 0)]
        protected Pin<DX11Resource<DX11DynamicStructuredBuffer>> FTextureOut;

        private List<VlcBufferPlayer> players = new List<VlcBufferPlayer>();

        #region Set Plugin Host
        [ImportingConstructor()]
        public VLCNodeBuffer(IPluginHost Host)
        {
            //assign host
            this.FHost = Host;
        ...
        public void Evaluate(int SpreadMax)
        {
            for (int i = 0; i < SpreadMax; i++)
            {
                ...
                if (needreset)
                {
                    if (File.Exists(path))
                    {
                        if (add)
                        {
                            this.players.Add(new VlcBufferPlayer());
                        }
                        else
                        {
                            this.players[i] = new VlcBufferPlayer();
                        }
            ...
            for (int i = 0; i < SpreadMax; i++)
            {
                ...
                if (this.FTextureOut[i] == null) { this.FTextureOut[i] = new DX11Resource<DX11DynamicStructuredBuffer>(); }
            }
            ...
            for (int i = 0; i < SpreadMax; i++)
            {
                if (this.players[i] != null)
                {
                    if (this.players[i].IsValid)
                    {
                        VlcBufferPlayer v = this.players[i];
                        v.GetStatus();
        ...
        {        
            try
            {
                foreach (VlcBufferPlayer tp in this.players)
                {
                    if (tp != null)
                    {
                        tp.Stop();
        ...
        public void Update(IPluginIO pin, DX11RenderContext context)
        {
            ...
            int cnt = 0;
            foreach (VlcBufferPlayer vlc in this.players)
            {
                ...
                if (vlc != null)
                {
                    if (vlc.IsValid)
                    {
                        DX11DynamicStructuredBuffer t;
                        if (!this.FTextureOut[cnt].Contains(context))
                        {
                            t = new DX11DynamicStructuredBuffer(context.Device, (vlc.Width * vlc.Height / 2) ,4);
                            this.FTextureOut[cnt][context] = t;
                        }
                        else
                        {
                            t = this.FTextureOut[cnt][context];
                            if (vlc.Width * vlc.Height * 2 != t.Size)
                            {
                                this.FTextureOut[cnt].Dispose(context);
                                DX11DynamicStructuredBuffer t2 = new DX11DynamicStructuredBuffer(context.Device, (vlc.Width * vlc.Height / 2), 4);
                        ...
                        if (vlc.Play)
                        {
                            ...
                            DataBox db = context.CurrentDeviceContext.MapSubresource(t.Buffer, SlimDX.Direct3D11.MapMode.WriteDiscard, SlimDX.Direct3D11.MapFlags.None);

                            memcpy(db.Data.DataPointer, ptr, vlc.Width * vlc.Height * 2);

                            context.CurrentDeviceContext.UnmapSubresource(t.Buffer, 0);
