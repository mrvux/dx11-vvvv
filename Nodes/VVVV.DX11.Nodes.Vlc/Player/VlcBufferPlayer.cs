...
{
    public unsafe class VlcBufferPlayer
    {
        ...
        private VlcVideoFormatSetupHandlerDelegate vlcVideoFormatSetupHandlerDelegate;
        private VlcVideoFormatCleanupHandlerDelegate vlcVideoFormatCleanupHandlerDelegate;
        ...
        private void Initialize()
        {
            ...
            vlcVideoFormatSetupHandlerDelegate = this.vlcFormatSetupCallBack;
            this.vlcVideoFormatCleanupHandlerDelegate = this.vlcCleanUpCallbal;
            media = new IntPtr();
        ...
        public void SetFileName(string file)
        {
            ...
            if (this.valid)
            {
                ...
                LibVlcMethods.libvlc_video_set_format_callbacks(mediaPlayer, Marshal.GetFunctionPointerForDelegate(vlcVideoFormatSetupHandlerDelegate),
                    Marshal.GetFunctionPointerForDelegate(vlcVideoFormatCleanupHandlerDelegate));

                /*int pitch = this.w * 2;
                LibVlcMethods.libvlc_video_set_format(mediaPlayer, Encoding.UTF8.GetBytes("YUYV"), this.w, this.h, pitch);*/
        ...
        public uint vlcFormatSetupCallBack(ref IntPtr pUserData, IntPtr chroma, IntPtr width, IntPtr height, uint* pitches,uint* lines)
        {
            byte[] b = new byte[4];
            Marshal.Copy(chroma, b, 0, 4);

            string ch = Encoding.UTF8.GetString(b);

            uint wi = Convert.ToUInt32(w);
            uint he = Convert.ToUInt32(h);
            uint planecount = 1;
            ...
            switch (ch)
            {
                case "I420":
                    planecount = 3;
                    pitches[0] = wi; pitches[1] = wi; pitches[2] = wi;
                    lines[0] = he; lines[1] = he; lines[2] = he;
                    break;
            }
            ...
            int pitch = this.w * 2;*/
            /*pitches[0] = Convert.ToUInt32(this.w * 2);
            lines[0] = Convert.ToUInt32(this.h);*/


            return planecount;
        }

        public void vlcCleanUpCallbal(IntPtr pUserData)
        {
        }
        ...
        private bool Realloc(int w, int h)
        {
            if (this.w != w || this.h != h)
            {
                ...
                this.buffer0 = Marshal.AllocHGlobal(w * h * 2 + 32);
                this.buffer1 = Marshal.AllocHGlobal(w * h * 2 + 32);
